...
class VentanaPrincipal:
    def __init__(self, master, store):
        self.master = master
        self.store = store  # Assign the store object to self.store
    ...
    def login(self):
        ...
        self.store = Store()  # Crear una nueva instancia de Store
        self.master.destroy()
...
class Ventana_Simulador(ctk.CTkToplevel):
    ...
    def add_product(self):
        ...
        print("Product Name:", name_val)  # Print the product name for verification
    ...
    def open_view_products_window(self):
        ...
        # Show only the names of the products
        for product in self.store.products:
            product_info = product.show_info()  # Call show_info function
            # Extract and display only the name from product_info
            product_name = product_info.split('\n')[1].split(': ')[1]
            ctk.CTkLabel(new_window, text=product_name).pack()
...
class Ventana_Simulador2(ctk.CTkToplevel):
    def __init__(self, master=None, perfil='', store=None):
        ...
        self.displaySale = ctk.CTkCanvas(self.master, bg = 'white')
        self.displaySale.pack(fill = ctk.BOTH, expand = True)

store = Store()  # Initialize the store object
root = ctk.CTk()
app = VentanaPrincipal(root, store)  # Pass the store object to VentanaPrincipal
root.mainloop()